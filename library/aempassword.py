...
class AEMPassword(object):
    ...
    def get_user_info(self):
	...
	#if status == 200:
        if status == 200 and len(info['hits']) != 0:
        ...
        if not old_password_valid:
            if self.ignore_err:
                ...
                self.exit_status()
            self.module.fail_json(msg='Cannot login with the password. Old password given could be invalid')
